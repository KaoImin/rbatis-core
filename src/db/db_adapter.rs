...
impl DBPool {
    ...
    pub fn make_db_query<'f, 's>(
        driver_type: &DriverType,
        sql: &'s str,
    ) -> crate::Result<DBQuery<'s>> {
        match driver_type {
            &DriverType::None => {
    ...
    pub async fn close(&self) {
        match &self.driver_type {
            ...
            #[cfg(feature = "mysql")]
            &DriverType::Mysql => match &self.mysql {
                None => {}
                Some(pool) => {
                    pool.close().await;
                }
            ...
            #[cfg(feature = "postgres")]
            &DriverType::Postgres => match &self.postgres {
                None => {}
                Some(pool) => {
                    pool.close().await;
                }
            ...
            #[cfg(feature = "sqlite")]
            &DriverType::Sqlite => match &self.sqlite {
                None => {}
                Some(pool) => {
                    pool.close().await;
                }
            ...
            #[cfg(feature = "mssql")]
            &DriverType::Mssql => match &self.mssql {
                None => {}
                Some(pool) => {
                    pool.close().await;
                }
...
impl DBPoolConn {
        ...
        T: DeserializeOwned,
    {
        ...
        match &self.driver_type {
            ...
            #[cfg(feature = "mysql")]
            &DriverType::Mysql => {
                let async_stream: Vec<MySqlRow> = self
                    .mysql
                    .as_mut()
                    .ok_or_else(|| Error::from("[rbatis-core] conn is none!"))?
                    .fetch_all(sql)
            ...
            #[cfg(feature = "postgres")]
            &DriverType::Postgres => {
                let async_stream: Vec<PgRow> = self
                    .postgres
                    .as_mut()
                    .ok_or_else(|| Error::from("[rbatis-core] conn is none!"))?
                    .fetch_all(sql)
            ...
            #[cfg(feature = "mssql")]
            &DriverType::Mssql => {
                let async_stream: Vec<MssqlRow> = self
                    .mssql
                    .as_mut()
                    .ok_or_else(|| Error::from("[rbatis-core] conn is none!"))?
                    .fetch_all(sql)
